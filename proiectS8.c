...
void afisareFisier(char *dir, char *dir2, char *nume){
    char buff[100];
    char path[100];
...
void parcurgeFisiere(char *dir, char *dir2){
    struct stat variabila;
    ...
    while((intrare = readdir(director)) !=NULL){
        ...
        if(pid == 0){
            if(intrare->d_type == DT_DIR){
                ...
                afisareFisier(dir,dir2,intrare->d_name);
                numarLinii = 8;
            }
            exit(numarLinii);
        }
